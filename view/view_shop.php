        ...
        <div class="card crad_tung card_new text-white" data-aos="fade-up">
            <div class="row p-2">
                <div class="col-6 text-center">
                    <img src="assets/img/game_icon/<?= $res_category['img'] ?>" alt="<?= $res_category['img'] ?>"
...
if (isset($_GET['category'])) {
    ...
    $stmt_shop = $connect->prepare("SELECT * FROM tbl_shop_id WHERE gameid = ? AND status = '1' AND shoptype = 'idgame' ORDER BY status ASC");
    $stmt_shop->bind_param('s', $idcategory);
    $stmt_shop->execute();
    $result = $stmt_shop->get_result();
} else {
...
if ($result->num_rows == 0) {
    ...
    foreach ($shop_fetchs as $shop_fetch):
        ...
        <div class="col-md-3 col-sm-12 col-12 px-2 d-flex align-items-stretch hvr-float pointer"
            onclick="CradURL('./shop/<?= $shop_fetch['id'] ?>')">
