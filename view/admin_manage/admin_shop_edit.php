...
$stmt = $connect->prepare("SELECT * FROM tbl_shop_id WHERE id = ?");
$stmt->bind_param('s', $idshop);
$stmt->execute();
$res = $stmt->get_result();
$result_shop_check = $res->num_rows;
$result_shop_edit = $res->fetch_assoc();
?>
...
<div class="col-md-12 col-sm-12 mt-2" data-aos="zoom-in">
    <div class="card crad_tung text-white mb-3">
        ...
        <div class="card-body">
            <a href="?page=manage&admin=manageshop&t=<?= $result_shop_edit['shoptype'] ?>" class="btn btn-danger"><i
                    class="fa-solid fa-circle-chevron-left"></i> กลับ</a>


            <div class="form-group row">
                <div class="form-group col-md-3">
                    <label for="formFile" class="form-label mt-2"><i class="fa-solid fa-chevron-right"></i>
                        ชื่อสินค้า</label>
                    <input class="form-control" type="text" id="shopname" placeholder="ชื่อสินค้า"
                        value="<?= $result_shop_edit['name'] ?>">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="shoptype" id="shoptype">
                        ...
                        <option value="1" <?php if ($result_shop_edit['shoptype'] == 'idgame') {
                            echo "selected";
                        } ?>>
                            ไอดีหรือรหัสเกม</option>
                        <option value="2" <?php if ($result_shop_edit['shoptype'] == 'account') {
                            echo "selected";
                        } ?>>บัญชี
                        </option>
                    </select>
                ...
                <div class="form-group col-md-2">
                    <label for="formFile" class="form-label mt-2"><i class="fa-solid fa-chevron-right"></i> หมวดหมู่ <a
                            href="?page=manage&admin=category"> เพิ่มหมวดหมู่</a></label>
                    <?php
                    ...
                    <select class="form-control" name="category" id="category">
                        ...
                        <?php foreach ($res_categorys as $res_category): ?>
                            <option value="<?= $res_category['id'] ?>" <?php if ($result_shop_edit['gameid'] == $res_category['id']) {
                                echo "selected";
                            } ?>>
                                <?= $res_category['name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group col-md-2">
                    <label for="formFile" class="form-label mt-2"><i class="fa-solid fa-chevron-right"></i>
                        ราคาปกติ</label>
                    <input class="form-control" type="text" id="point" placeholder="ราคา"
                        value="<?= $result_shop_edit['point'] ?>">
                </div>
                <div class="form-group col-md-2">
                    <label for="formFile" class="form-label mt-2"><i class="fa-solid fa-chevron-right"></i>
                        ราคาพิเศษ</label>
                    <input class="form-control" type="text" id="pointv" placeholder="ราคา"
                        value="<?= $result_shop_edit['pointv'] ?>">
                </div>
            </div>

            <div class="form-group row">
                <div class="form-group mb-2 col-md-6">
                    <label class="col-form-label" for="inputDefault"><i class="fa-solid fa-chevron-right"></i>
                        ข้อมูลสินค้าแสดงหน้าเว็บ</label>
                    <textarea id="name" class="summernote" name="name"
                        rows="5"><?= $result_shop_edit['publish_info'] ?></textarea>
                </div>
                <div class="form-group mb-2 col-md-6">
                    <label class="col-form-label" for="inputDefault"><i class="fa-solid fa-chevron-right"></i> ข้อมูลลับ
                        <span class="badge rounded-pill bg-danger" style="font-size: 0.9rem;">
                            เห็นเฉพาะผู้ซื้อและผู้ขายเท่านั้น!!</span></label>
                    <?php $infosecret = openssl_decrypt($result_shop_edit['secret_info'], $ciphering, $decryption_key, $options, $decryption_iv); ?>
                    <textarea id="secret_info" class="form-control" name="secret_info" rows="3"
                        placeholder="ตัวอย่าง id:asdzxc123 pass:123465"><?= $infosecret ?></textarea>
            ...
            <div class="form-group row">
                ...
                <div class="form-group mb-3 col-md-6 mt-4">
                    <button onclick="history.go(0)" class="btn btn-danger w-100 mb-2"><i
                            class="fa-solid fa-rotate-right"></i> ย้อนกลับ</button>
                    <button onclick="ok_shop()" class="btn btn-primary w-100"><i class="fa-solid fa-circle-check"></i>
                        แก้ไขข้อมูล</button>
                </div>
                ...
                foreach ($result_shop as $i => $result_shops):
                    if (!empty($result_shops)):
                        ...
                        <div class="form-group mb-2 col-md-3" id="showImgId<?= $i ?>">
                            ...
                            <button class="btn btn-danger w-100 mb-2 mt-2"
                                onclick="deleteImg('<?= $result_shops ?>',<?= $i ?>)">ลบ</button>
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
