<div class="col-md-6 col-sm-12 mt-2" data-aos="zoom-in">
    <div class="card crad_tung text-white mb-3">
        <div class="card-header"><i class="fa-solid fa-caret-right" style="color:#F186A9;"></i> Manage Category -
            จัดการหมวดเกม</div>
        <div class="card-body">
            ...
            $stmt = $connect->prepare("SELECT * FROM tbl_game WHERE id = ?");
            $stmt->bind_param('s', $id);
            $stmt->execute();
            $res = $stmt->get_result();
            $result_category_check = $res->num_rows;
            $result_category_edit = $res->fetch_assoc();
            if ($result_category_check == 0):
                ?>
                <div class="alert alert-dismissible alert-success">
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                    <strong>Well done!</strong>successfully. you can <a href="?page=manage&admin=category"
                        class="alert-link">Add new category</a>.
                </div>
            <?php else: ?>

                <div class="form-group">
                    <div class="text-center pointer">
                        <img src="assets/img/game_icon/<?= $result_category_edit['img'] ?>" alt="null" width="20%" id="output"
                            onclick="click_image()">
                    </div>
                </div>
                <div class="form-group">
                    <label for="formFile" class="form-label mt-4">รูปหมวดเกม <span class="badge rounded-pill bg-warning"
                            style="font-size: 0.9rem;">ขนาด 200x200px</span></label>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" id="name" placeholder="pung"
                        value="<?= $result_category_edit['name'] ?>">
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control" name="typecategory" id="typecategory">
                        ...
                        <option value="idgame" <?php if ($result_category_edit['typegame'] == 'idgame') {
                            echo "selected";
                        } ?>>
                            ไอดีเกม</option>
                        <option value="account" <?php if ($result_category_edit['typegame'] == 'account') {
                            echo "selected";
                        } ?>>
                            บัญชี</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" id="platform" placeholder="Steam"
                        value="<?= $result_category_edit['platform'] ?>">
                </div>

                <div class="form-group mt-4">
                    <a onclick="ok_category()" class="btn btn-info w-100"><i class="fa-solid fa-folder-plus"></i> Edit
                        Category - อัพเดทหมวดเกม</a>
                </div>
            <?php endif; ?>
...
<div class="col-md-6 col-sm-12 mt-2">
    <div class="card crad_tung text-white mb-3">
        <div class="card-header"><i class="fa-solid fa-caret-right" style="color:#F186A9;"></i> Category - หมวดเกมที่มี
        </div>
        <div class="card-body">
            ...
            <table width="100%">
                ...
                foreach ($res_categorys as $res_category):
                    ...
                    <tr>
                        ...
                        <td style="text-align:center"><img src="assets/img/game_icon/<?= $res_category['img']; ?>"
                                alt="<?= $res_category['img']; ?>" width="50px"></td>
                        <td style="text-align:left"><?= $res_category['name']; ?></td>
                        <td style="text-align:right">
                            <a href="?page=manage&admin=category_edit&id=<?= $res_category['id']; ?>"
                                class="hvr-icon-up btn btn-info btn-sm"><i class="hvr-icon fa-solid fa-pen-to-square"></i>
                                แก้ไข</a>
                            <a onclick="delete_category(<?= $res_category['id']; ?>)"
            ...
            <div class="mt-3">
                <a href="?page=manage&admin=category" class="hvr-icon-up btn btn-info btn-sm w-100"><i
                        class="hvr-icon fa-solid fa-plus"></i> เพิ่มหมวดเกม</a>
            </div>
        </div>
    </div>
</div>
